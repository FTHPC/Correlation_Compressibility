...
extract_cr_predictors <- function(data, error_mode, error_bnd, gaussian_corr=0, sz3=TRUE, comp_thresh=200, quant=1, qbins=1, dim3D=FALSE){
  data_orig <- filter(data, info.bound_type == error_mode)
  data_orig <- filter(data_orig, info.error_bound==error_bnd)
  if (quant==1) {data0 <-  filter(data_orig, info.quantized=='False' | info.quantized=='FALSE')}
  if (quant==0) {data0 <-  data_orig}
  ###
  ...
  ###
  if (dim3D == TRUE){compressor0 <- c(compressor0, 'tthresh')}
  ###
  list_df_cr <- NULL
  ...
  if (qbins==1){ vx_compressor <- filter(vx_compressor, sz.quantization_intervals=="default") }
  indsz <- which(as.numeric(vx_compressor$size.compression_ratio)<=comp_thresh)
...
hosvd_trunc <- function(X){
  ...
  ### select significant singular modes
  svd_trunc <- c()
  sv0 <- c(sv_list[[1]][1:2], sv_list[[2]][1:2], sv_list[[3]][1:2])
  sv1 <- c(sv_list[[1]][3:dim(tnsr)[1]], sv_list[[2]][3:dim(tnsr)[2]], sv_list[[3]][3:dim(tnsr)[3]])
  sv_sort <- sort(sv1, index.return=TRUE, decreasing = TRUE)
  total_contrib <- cumsum(c(sv0, sv_sort$x)^2)/sum(c(sv0, sv_sort$x)^2)
  svd_trunc[1] <- min(which(total_contrib>=max(1,total_contrib[6])))
  svd_trunc[2] <- min(which(total_contrib>=max(0.99,total_contrib[6])))
  svd_trunc[3] <- min(which(total_contrib>=max(0.95,total_contrib[6])))
  svd_trunc[4] <- min(which(total_contrib>=max(0.9,total_contrib[6])))   
  #colnames(svd_trunc) <- c('n100', 'n99', 'n95', 'n90')
  return(svd_trunc)
}
...
cr_regression_gam <- function(df, kf=8, graph=1, fig_nm, data_nm, compressor_nm, error_mode, error_bnd, block_count=block_count, block_size=block_size, print_stats=1){
  ...
  if (nrow(df_reg) == 0){
    print(paste("no work: ", compressor_nm, error_mode, error_bnd))
  }
...
cr_regression_gam_mixture <- function(df, k=2, kf=8, graph=1, fig_nm, data_nm, compressor_nm, error_mode, error_bnd, print_stats=1){
  indsz <- which(df$y<=comp_thresh)
  df <- df[indsz,]
  qent0 <- df$qent
  qent <- (log(qent0)-min(log(qent0),na.rm=TRUE))/(max(log(qent0),na.rm=TRUE)-min(log(qent0),na.rm=TRUE))
  ...
  for (l in 1:kf){
    test.set <- df_reg[df_reg$fold == l,]
    train.set <- df_reg[df_reg$fold != l,]
    mi <- flexmix(y ~ qent + vrgstd + qent*vrgstd, k=k, data = train.set)
    predi <- c(unlist(predict(mi, newdata=test.set, aggregate=TRUE)))
    #
    cv_mape[l] <- RelMAE(true=exp(test.set$y),pred=exp(predi))
    cv_cor[l] <- cor(exp(test.set$y),exp(predi))
    #
  ...
  if (graph == 1){
    ...
    graphics.off()
    png(filename = paste(fig_nm,'_scatterplot_cv_gam_mixture_',compressor_nm,'_',data_nm,'_',error_mode,error_bnd,'.png', sep=''), width = 400, height = 400)
    plot(ytest_list[[1]], pred_list[[1]], xlim=c(ymin, ymax), ylim=c(ymin, ymax), pch=20, cex=1.2,  cex.lab=1.45, cex.axis=1.4, cex.main=1.6,col=Col[1], xlab='Observed CR', ylab='Predicted CR', main=paste(data_nm,' - ',compressor_nm,error_mode,error_bnd), mgp=c(2,0.5,0))
    abline(0, 1)
    #
    for (i in 1:kf){
