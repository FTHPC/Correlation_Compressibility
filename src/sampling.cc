...
/*
  ...
  sampling methods
*/
std::vector<std::shared_ptr<loader>> block_sampler::sample(int rank, int nprocs, usi method_d, usi total_blocks, size_t block_dims) {
  this->rank = rank;
  this->nprocs = nprocs;
  switch (method_d) {
    case(UNIFORM):
      ...
      for (size_t i=0; i<meta->dims[0]-block_dims; i+=block_dims)  // goes along x dimension 
        for (size_t j=0; j<meta->dims[1]-block_dims; j+=block_dims) // goes along y dimension
          for (size_t k=0; k<meta->dims[2]-block_dims; k+=block_dims) { // goes along z dimension
            std::cout << "rank" << rank << ":" << block_num << std::endl;
            // new block created
